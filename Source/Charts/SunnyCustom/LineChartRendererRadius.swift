...
open class LineChartRendererRadius: LineRadarRenderer
{
    ...
    @objc open func drawLinear(context: CGContext, dataSet: LineChartDataSetProtocol)
    {
        ...
        { // only one color per dataset
            ...
            // 最小值
            let yMin: CGFloat = dataSet.fillFormatter?.getFillLinePosition(dataSet: dataSet, dataProvider: dataProvider) ?? 0.0
            let radius = 1.0
            ...
            for x in stride(from: _xBounds.min, through: _xBounds.range + _xBounds.min, by: 1)
            {
                ...
                // 需要针对 第一点 和 最后一点做处理
                if x == 0 {
                    let zeroPoint = CGPoint(x: CGFloat(e1.x), y: CGFloat(yMin)).applying(valueToPixelMatrix)
                    path.move(to: zeroPoint)
                    path.addArc(tangent1End: zeroPoint, tangent2End: prevPoint, radius: radius)
                    firstPoint = false
                    continue
                ...
                let steppedPoint =
                    CGPoint(
                        ...
                        y: CGFloat(e1.y * phaseY))
                    .applying(valueToPixelMatrix)
                path.addArc(tangent1End: prevPoint, tangent2End: steppedPoint, radius: radius)
                ...
                let endPoint =
                    ...
                    .applying(valueToPixelMatrix)
                path.addArc(tangent1End: steppedPoint, tangent2End: endPoint, radius: radius)
                
                if x == entryCount-1 {
                    let zeroPoint = CGPoint(x: CGFloat(e2.x), y: CGFloat(yMin)).applying(valueToPixelMatrix)
    ...
    private func generateFilledPath(dataSet: LineChartDataSetProtocol, fillMin: CGFloat, bounds: XBounds, matrix: CGAffineTransform) -> CGPath
    {
        ...
        for x in stride(from: (bounds.min + 1), through: bounds.range + bounds.min, by: 1)
        {
            ...
            if isDrawSteppedEnabled
            {
                ...
                let step = CGPoint(x: CGFloat(e.x), y: CGFloat(ePrev.y * phaseY)).applying(matrix)

                let start = CGPoint(x: CGFloat(ePrev.x), y: CGFloat(ePrev.y * phaseY)).applying(matrix)
                filled.addArc(tangent1End: start, tangent2End: step, radius: radius)
                
                let end = CGPoint(x: CGFloat(e.x), y: CGFloat(e.y * phaseY)).applying(matrix)
                filled.addArc(tangent1End: step, tangent2End: end, radius: radius)
